...
conivalmap(EvalWorkspace *ew, int i, real *X, fint *nerror)
{
	...
	if ((cm = a->i.cmap))
		i = cm[i];
	return a->p.Conival_nomap(ew, i, X, nerror);
	}
...
congrdmap(EvalWorkspace *ew, int i, real *X, real *G, fint *nerror)
{
	...
	if ((cm = a->i.cmap))
		i = cm[i];
	a->p.Congrd_nomap(ew, i, X, G, nerror);
	}
...
read_line(EdRead *R)
{
 ...
 void
memfailure_ASL(const char *who, const char *what, size_t len)
{
	if (progname)
		fprintf(Stderr, "%s: ", progname);
	fprintf(Stderr, "%s(%lu) failure: %s.\n", who, (unsigned long)len, what);
	exit(1);
	}
...
mymalloc(size_t len)
{
	...
	if (!rv) {
		...
		if (!rv)
			memfailure_ASL(who, ran_out, len);
		}
	return rv;
	}
...
xknown_(real *x)
{
	...
	if (!(ws = asl->i.Ew0))
		asl->i.Ew0 = ws = asl->p.EWalloc(asl);
	xknowne_ew(ws, x, 0);
	}
...
xknowe_(real *x, fint *nerror)
{
	...
	if (!(ws = asl->i.Ew0))
		asl->i.Ew0 = ws = asl->p.EWalloc(asl);
	xknowne_ew(ws, x, nerror);
	}

 void
xunknown_ASL(EvalWorkspace *ew)
{
...
prob_adj_ASL(ASL *asl)
{
 ...
 badnumber:
	...
	asl->i.pewthread = &asl->i.ewthread;
	if (!(flags & ASL_no_ew0_alloc))
		asl->i.Ew0 = (*asl->p.EWalloc)(asl);
