...
obj_adj1(ASL *asl, int no, int *pco, cgrad **pcgo, real *prhs)
{
	...
	int *Cvar, *cm, *cmax, *o, *omax, *vm, *zg, **zgp;
	ograd *og;
        ps_func *P;
        ps_func2 *P2;
	real *Lc, *Lv, *Uc, *Uv, c1, c2, oc, *pi, rhs, t;

	cclass = oclass = 0;
	cmax = omax = 0;
	switch (asl->i.ASLtype) {
	 case ASL_read_fg:
		o = ((ASL_fg*)asl)->I.obj_de_[no].o.e;
		break;
	 case ASL_read_fgh:
		o = ((ASL_fgh*)asl)->I.obj2_de_[no].o.e;
		break;
         case ASL_read_pfg:
		P = &((ASL_pfg*)asl)->P.ops[no];
		if (P->pi.b || P->g)
			return;
		o = ((ASL_pfg*)asl)->I.obj_de_[no].o.e;
	 ...
	 case ASL_read_pfgh:
		...
		if (P2->pi.b || P2->g)
			return;
		o = ((ASL_pfgh*)asl)->I.obj2_de_[no].o.e;
	...
	if (o) {
		if (*o)
			return;
		oc = ((real*)((char*)asl->i.numvals + asl->i.numlen))[o[1]];
		}
	og = Ograd[no];
	if (!og || og->next)
		return;
	...
	od->ivo = cv;
	od->c0 = oc;
	asl->i.objconst[no] += od->c0a = oc + t*rhs;
	od->c1 = c1;
	od->c12 = -t;
	od->nxval = -1;
...
objval_adj(EvalWorkspace *ew, int no, real *X, fint *nerror)
{
	...
	asl = ew->asl;
	if (!(od = asl->i.Or[no]))
		return asl->p.Objval_nomap(ew, no, X, nerror);
	if ((cg = od->cg)) {
		if (!(gr0 = asl->i.Cgrad0))
		...
		cg0 = *pcg;
		*pcg = cg;
		c = asl->p.Conival_nomap(ew, od->ico, X, nerror);
		*pcg = cg0;
		}
	else
		c = asl->p.Conival_nomap(ew, od->ico, X, nerror);
...
objgrd_adj(EvalWorkspace *ew, int no, real *X, real *G, fint *nerror)
{
	...
	asl = ew->asl;
	if (!(od = asl->i.Or[no])) {
		asl->p.Objgrd_nomap(ew, no, X, G, nerror);
		return;
		}
	if (od->nxval != ew->nxval)
		objval_adj(ew, no, X, nerror);
	if ((k = asl->i.congrd_mode))
		asl->i.congrd_mode = 0;
	asl->p.Congrd_nomap(ew, od->ico, X, G, nerror);
	asl->i.congrd_mode = k;
...
paradj(EvalWorkspace *ew, int *pno, real **pow, real **py)
{
	...
	Or = asl->i.Or;
	if (!(ws = ew->orscratch))
		ew->orscratch = ws = (real*)M1zapalloc((asl->i.n_con0 + nobj)*sizeof(real));
...
sphese_adj(EvalWorkspace *ew, SputInfo **spi, real *H, int no, real *ow, real *y, fint *nerror)
{
 ...
 static fint
sphes_setup_adj(EvalWorkspace *ew, SputInfo **spi, int no, int ow, int y, int uptri)
{
	ASL *asl;
	Objrep *od, **odp;
	...
	return asl->p.Sphset_nomap(ew, spi, no, ow, y, uptri);
	}
		...
		if ((od = odp[no])) {
			...
			if (od->nxval != ew->nxval || od->cg0) {
				...
				if (od->cg0)
					...
					*cgp = od->cg;
				od->f = objval_adj(ew, no, x0, &nerror);
				*cgp = cgsave;
				if (nerror)
					continue;
				}
